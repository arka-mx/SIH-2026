...
export default function RescuerDetailPage({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const unitProfile = stored ?? profileFromSession(rescuerId, s);
      setProfile(unitProfile);

      if (s.isTeamHead) {
        apiRegisterTeamHeadContact({
          teamId: rescuerId,
          headName: unitProfile.leaderName || s.name || s.email,
          headPhone: unitProfile.phone || "+91 98765 11001",
          headOffice: s.officeName || unitProfile.name,
        });
      }
